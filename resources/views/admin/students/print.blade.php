...
<head>
    <meta charset="utf-8">
    <title>Fiche {{ $eleve->displayId() }} · {{ config('app.brand') }}</title>
    <style>
...
<body>
    <div class="header"><div><div class="logo">{{ config('app.brand') }}</div><div>Fiche technique élève</div></div><div class="id">{{ $eleve->displayId() }}</div></div>
    <div class="profile">
        @if($eleve->photo_url)<img src="{{ $eleve->photo_url }}" alt="" class="photo">@else<div class="photo">Photo</div>@endif
        <div><h1>{{ $eleve->nom_complet }}</h1><p>{{ $eleve->niveau_scolaire }} · {{ $eleve->etatLabel() }}</p></div>
    </div>
    <div class="grid">
        @foreach([
            'Contact élève' => $eleve->contact,
            'Nom du tuteur' => $eleve->tuteur_nom ?: '—',
