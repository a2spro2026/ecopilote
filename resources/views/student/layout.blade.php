...
<div class="flex min-h-screen">
    <aside id="studentSidebar" class="fixed inset-y-0 left-0 z-40 flex w-[270px] -translate-x-full flex-col border-r border-slate-200 bg-white transition-transform duration-300">
        ...
        <div class="border-b border-slate-100 p-4">
            <a href="{{ route('student.room') }}" data-mdi-skip class="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-500 px-3 py-2.5 text-sm font-bold text-white shadow-sm">Entrer dans la salle</a>
        </div>
        <nav class="flex-1 space-y-1 overflow-y-auto p-3">
            @foreach($nav as $item)
                @php $active = request()->routeIs($item['route']); @endphp
                <a href="{{ route($item['route']) }}" data-window-title="{{ $item['label'] }}" class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold transition {{ $active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
    ...
    <div id="studentMain" class="flex h-screen flex-1 flex-col overflow-hidden transition-[padding] duration-300 ease-out">
        <header class="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6">
            @include('partials.sidebar-toggle', ['tone' => 'student', 'onclick' => 'toggleStudentSidebar()', 'controls' => 'studentSidebar'])
            <div class="min-w-0 flex-1"><h1 class="truncate text-base font-bold text-slate-900 sm:text-lg" style="font-family:Poppins,sans-serif">Mon espace</h1><p class="hidden text-xs text-slate-500 sm:block">Les pages s’ouvrent en fenêtres, comme un bureau de travail</p></div>
            <a href="{{ route('student.room') }}" data-mdi-skip class="hidden rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white sm:inline-flex">Retour à la salle</a>
            ...
            @include('partials.school-logout', ['action' => route('student.logout'), 'tone' => 'student'])
...
<script>
const STUDENT_SIDEBAR_KEY = 'ecopilote.student.sidebarOpen';
...
function setStudentSidebarOpen(open) {
    ...
    if (isDesktop()) {
        ...
        main.style.paddingLeft = open ? '270px' : '0px';
        localStorage.setItem(STUDENT_SIDEBAR_KEY, open ? '1' : '0');
    } else {
...
function toggleStudentSidebar(force) {
    const currentlyOpen = document.getElementById('studentSidebar').classList.contains('translate-x-0');
    setStudentSidebarOpen(typeof force === 'boolean' ? force : !currentlyOpen);
}
setStudentSidebarOpen(isDesktop() ? localStorage.getItem(STUDENT_SIDEBAR_KEY) !== '0' : false);
window.addEventListener('resize', () => setStudentSidebarOpen(isDesktop() ? localStorage.getItem(STUDENT_SIDEBAR_KEY) !== '0' : false));
document.addEventListener('ecopilote:close-sidebar', () => setStudentSidebarOpen(false));
document.addEventListener('click', e => {
    const wrap = document.getElementById('studentUserWrap');
    if (wrap && !wrap.contains(e.target)) document.getElementById('studentUserMenu')?.classList.add('hidden');
});
</script>
</body>
</html>
