            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($candidatures as $c)
                    ...
                    <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 {{ $isValidee ? 'opacity-70' : '' }}">
                        ...
                        <td class="truncate text-slate-600 dark:text-slate-300" title="{{ $c->contact }}">{{ $c->contact }}</td>
                        <td class="truncate text-slate-600 dark:text-slate-300" title="{{ $c->ville }}">{{ $c->ville }}</td>
                        <td class="truncate text-slate-600 dark:text-slate-300" title="{{ $c->niveauLabel() }}">{{ $c->niveauLabel() }}</td>
                        <td class="text-[12px] font-semibold text-slate-700 dark:text-slate-200" title="{{ $c->matiere }}">{{ \App\Support\SubjectAbbreviation::display($c->matiere) }}</td>
