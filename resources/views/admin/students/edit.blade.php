...
<form method="POST" action="{{ route('admin.students.update', $eleve) }}" class="max-w-3xl space-y-4">
    ...
    <section class="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="grid gap-3 sm:grid-cols-2">
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Identifiant</label>
                <div class="flex overflow-hidden rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                    <input type="text" value="{{ \App\Support\EcopiloteIdentity::localPart($eleve->login ?: \App\Support\EcopiloteIdentity::loginFromName($eleve->nom_complet)) }}" readonly class="min-w-0 flex-1 border-0 bg-transparent px-3 py-2 text-sm font-semibold outline-none">
                    <span class="flex items-center border-l border-slate-200 bg-slate-100 px-3 text-xs font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">{{ \App\Support\EcopiloteIdentity::emailSuffix() }}</span>
                </div>
            </div>
            <div>
            ...
            <div>
                <label class="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Nom Tuteur</label>
                <input type="text" name="tuteur_nom" value="{{ old('tuteur_nom', $eleve->tuteur_nom) }}"
                       class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-800">
            </div>
            <div>
